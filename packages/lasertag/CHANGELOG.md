...
- 7c98dab: Support idiomatic Hono JSX with Hono-native custom-element types, render-story analysis for fragments and suspense/error boundaries, and CSS Module examples and guidance for server-rendered pages and HTMX fragments.

## 0.8.3

### Patch Changes

- 540e879: This patch fixes cases where `lasertag fix` and editor cleanup could remove styles for elements that still render. These examples come from the new [JSX runtime tests](https://github.com/jeremybanka/lasertag/blob/540e879d54369dff3f0ff1dcc575d76b5be90aea/packages/lasertag/tests/public/fixtures/jsx-cleanup-safety.ts) and [Solid tests](https://github.com/jeremybanka/lasertag/blob/540e879d54369dff3f0ff1dcc575d76b5be90aea/packages/lasertag/tests/public/fixtures/solid-switch.ts).
...
- ede5050: Document that innerHTML and dangerouslySetInnerHTML are outside Lasertag's analysis guarantees, including CSS cleanup.
...
- 67d81e3: Add zero-runtime, instance-local `@lasertag-own-subtree` annotations that validate consumer CSS against resolvable imported-component render stories, including original JSX or TSX discovered through declaration source maps, while preserving nested ownership boundaries and reporting unavailable adoption evidence.
